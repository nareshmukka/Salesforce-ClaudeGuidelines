...
Authoritative grammar and workflow for Salesforce metadata deployment in this project, using `sf` CLI v2. Other skill files (`apex_guidelines.md`, `flow_guidelines.md`, `lwc_guidelines.md`, `agentforce_authoring_bundle_guide.md`) reference this one for any `sf project deploy` invocation.
...
> **Project rule (PlusGradeFullSB):** Flows deploy as `status = Draft`. Naresh activates them manually after sandbox validation. Old flows are NOT deactivated by AI agents. See §11.

---

## 1. Project Constants (Read Before Composing Any Command)
...
| **Tests** | `--test-level <level>` | See §6 for values |
...
**Execution:** confirm the issue is real → retrieve current state for investigation → deploy prior version → run §14 smoke tests → document the incident (add a §20 Empirical Findings row if a new pattern emerged).
...
7. **Flow status check** — confirm every flow in the manifest has `<status>Draft</status>` (project rule §11).
8. **Smoke test plan** — specific checks matched to the deployed components.
...
- Project policy: `CLAUDE.md` §3 (Case flow migration rules — Draft status, no AI activation, baseline test failures)
