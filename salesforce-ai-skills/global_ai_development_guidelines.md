...
|---|---|
| Apex class — service, batch, queueable, scheduled, invocable | `apex_guidelines.md` |
| Apex trigger and trigger handler | `trigger_guidelines.md` |
| Apex test class, Jest spec, test data factory | `testing_guidelines.md` |
| Flow — record-triggered, autolaunched, screen, subflow | `flow_guidelines.md` |
| Lightning Web Component | `lwc_guidelines.md` |
| Custom object, field, validation rule, CMDT, record type | `metadata_guidelines.md` |
| Permission set, Permission Set Group, Muting PS | `permission_set_guidelines.md` |
| Callout, Named Credential, Platform Event, CDC, integration | `integration_guidelines.md` |
| Logging, error tracking, AppLog, correlation IDs | `observability_logging_guidelines.md` |
| Deployment, package.xml, validation, rollback | `deployment_guidelines.md` |
| Email template | `email_template_guidelines.md` |
| Prompt Builder template | `prompt_template_guidelines.md` |
| Lightning App Builder page (FlexiPage) | `flexipage_guidelines.md` |
| Visualforce page or controller | `visualforce_guidelines.md` |
| Agentforce agent topic / instruction (Builder UI) | `agentforce_script_guidelines.md` |
| Agentforce Builder metadata (Bot, BotVersion, GenAiPlannerBundle) | `agentforce_builder_guidelines.md` |
| Agentforce `.agent` DSL — authoring bundle, lifecycle | `agentforce_authoring_bundle_guide.md` |
| Agent Script grammar reference (read-only canonical) | `agentforce-agent-script-reference.md` |
| Agentforce Service Assistant on Case (this org) | `service_assistant_guidelines.md` |
...
- The Logger (see `observability_logging_guidelines.md`) is the only path to `AppLog__c`.
...
Details: `integration_guidelines.md`.
...
Component-specific patterns (selector boilerplate, `@AuraEnabled` controllers, REST endpoints) live in `apex_guidelines.md`.
...
|---|---|
| Logging | Never log: passwords, API keys, OAuth tokens, full credit-card numbers, government IDs, full email body content. Log: record Id, object type, operation, correlation Id, error message. See `observability_logging_guidelines.md`. |
...
1. **No hardcoded org-specific IDs** — record types, queues, profiles, users, groups, record IDs. Always look up at runtime via Schema methods or query by developer name. See §6.
2. **No hardcoded endpoints, credentials, or secrets** — every URL behind a Named Credential, every secret in External Credentials or platform credential stores. See §4.3.
3. **No SOQL or DML inside loops** — collect IDs first, query once, build a Map, iterate over the Map. See `apex_guidelines.md` for the canonical pattern.
4. **No unilateral destructive operations** — do not delete metadata, deactivate flows, drop permission sets, or remove fields without explicit user direction. The Case Flow Migration project specifically forbids deactivating old flows and activating new flows (see [CLAUDE.md §3](../CLAUDE.md)).
...
7. **No `@isTest(SeeAllData=true)` without a documented justification** — test classes own their data. See `testing_guidelines.md`.
8. **No `WITH SECURITY_ENFORCED` in new code** — use `WITH USER_MODE`. See §4.2.
9. **No missing sharing keyword on Apex classes** — every class declares one. See §4.1.
...
Flow variable naming: `inputX`, `outputX`, `colObjectRecords`, `loopObjectRecord`, descriptive camelCase for primitives. Details in `flow_guidelines.md`.
...
- [ ] No hardcoded record-type IDs, queue IDs, user IDs, profile names, endpoints, or secrets (§5, §6).
- [ ] Named Credentials used for every HTTP callout.
- [ ] Custom Metadata Types used for non-secret per-environment configuration.
- [ ] PII handling baseline (§4.4) satisfied.

### 8.3 Architecture & Code Quality

- [ ] Standard Apex doc-block header (§1.1) present on every new/modified class.
- [ ] Layering (§3) respected — no business logic in triggers, LWC JS, or VF controllers.
- [ ] Naming conventions (§7) applied throughout.
...
- [ ] Logging added for errors, integration calls, escalations (see `observability_logging_guidelines.md`).
...
- [ ] **Project exception:** Case Flow Migration defers test classes per CLAUDE.md §3 (`testsDefault: deferred`). Resume tests after sandbox functional testing.

### 8.5 Release Readiness

- [ ] Deployment order correct (see `deployment_guidelines.md`).
...
- [ ] **Project rules from CLAUDE.md §3 honored:** new flows deployed as `Draft` (not active); old flows not deactivated by the agent.
...
| Activating new flows or deactivating old flows (Case Flow Migration) | Explicitly forbidden by CLAUDE.md §3 — Naresh handles activation manually after sandbox validation |
...
7. **Cite the skill files in play.** "Applying §4.2 of `global_ai_development_guidelines.md` and §5 of `apex_guidelines.md`."
...
When a process-level finding emerges — something about how Salesforce documentation, CLI tooling, or AI-agent behavior interacts with this project — it goes here. Component-specific findings go to the matching skill file (see [CLAUDE.md §6](../CLAUDE.md) routing table).
...
Reactive ledger — each row records a mistake actually made by an AI agent or developer against the rules above, and the corrected approach. Component-specific mistakes (specific Apex / Flow / LWC patterns) go into the matching skill file per the routing table in [CLAUDE.md §6](../CLAUDE.md).

| # | Mistake | Correct approach |
|---|---|---|
| 1 | Hardcoding record type IDs, queue IDs, user IDs, or profile IDs | Look up at runtime via Schema methods or query by developer name (§6) |
| 2 | Hardcoding endpoints, API keys, tokens, passwords, or any credentials | Named Credential for endpoints; External Credential for auth; CMDT for non-secret config (§4.3, §6) |
| 3 | Placing SOQL or DML inside a loop | Collect IDs first, query once, build a Map, iterate the Map outside the loop (§5 hard-stop #3) |
| 4 | Placing business logic in a trigger body, LWC JS, or VF controller | Delegate to a service class; entry layer is for surface validation and delegation only (§3) |
| 5 | Using `@isTest(SeeAllData=true)` without documented justification | Use a TestDataFactory; tests own their data |
...
| 7 | Using `without sharing` without a documented justification | Default to `with sharing` or `inherited sharing`; document any escalation in a class comment (§4.1) |
| 8 | Editing profiles to grant feature access instead of using permission sets | Profiles are baseline only; features are gated by Permission Sets / PSGs (§7.4) |
...
| 16 | Omitting the sharing keyword from an Apex class | Implicit `without sharing` behaviour in most contexts; silent security regression (§4.1) |
...
| 19 | Activating new flows or deactivating old flows on the Case Flow Migration project | Forbidden by CLAUDE.md §3 — Naresh handles activation manually after sandbox validation. Deploy new flows as `Draft`. |
...
**Plan and scope**
- [ ] Plan from §10 was followed — no scope creep, no undocumented changes.
...
**Definition of Done**
- [ ] Every applicable item from §8 checked.

---

## 14. Official References

Authoritative sources for the rules in this file. When a rule here conflicts with a more recent release note, the release note wins — and an empirical-findings row gets added to §11.
