...
description: Production Salesforce AI skill for Any trigger or trigger handler change.
license: Apache-2.0
compatibility:
  - Claude Code
  - Claude Agents
...
## TRIGGER when
- The task involves Any trigger or trigger handler change.
...
- Also read: Global Development + Apex + Testing.
...
One trigger per object; trigger delegates to handler only. Use context-specific methods (beforeInsert, afterUpdate). Recursion guard via change detection/idempotent logic, not blind static flags.

## Examples
### Good example patterns
1. Trigger only instantiates handler and passes Trigger.new/oldMap.
2. After update logic processes only changed records using field-diff checks.

### Bad examples / avoid
1. Business logic directly in trigger body.
2. SOQL query inside per-record loop.
...
## Trigger-specific implementation rules
- One trigger per object; no business logic in trigger file.
- Separate before/after contexts in handler methods.
- Never perform SOQL/DML inside loops.
- Use recursion guard via change-detection/idempotent checks, not fragile globals alone.
- Add bulk tests for insert/update/delete/undelete where relevant.
...
- Name `TA_<SObject>_<Concern>` (see Section 15 Recursion in TAF:** prefer field-value comparison (Section 7 3) over static booleans -- TAF's per-action structure makes shared static flags awkward.
...
|---|---|
| After-update writes to same record | Trigger fires -> service updates Case -> trigger fires again -> ˆž |
