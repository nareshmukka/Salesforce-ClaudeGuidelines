...
description: Production Salesforce AI skill for Plusgrade Agentforce Service Assistant on Case work.
license: Apache-2.0
compatibility:
  - Claude Code
  - Claude Agents
...
## TRIGGER when
- The task involves Plusgrade Service Assistant on Case, ServicePlanner agents, Case assistant UI, or related Agentforce troubleshooting.
...
## DO NOT TRIGGER when
- The task is unrelated to Service Assistant on Case or Case-scoped Agentforce patterns.
- Agentforce Script, Builder metadata, or authoring bundle work is the primary owner and Service Assistant is only incidental.
...
- Also read: Agentforce Script + Agentforce Builder + Agentforce Authoring Bundle.
- Deep guidance is included in this `SKILL.md`.

## Purpose
Provide independently usable, production-ready assistant guidance for Plusgrade's Service Assistant on Case implementation without relying on external memory.
...
Service Assistant on Case uses the ServicePlanner agent pattern, not the `.agent` DSL. Keep ServicePlanner work separate from server-invoked EmployeeAgent implementations such as Email Resend / Email Analysis, verify rep and bot user permissions separately, and preserve explicit approval gates for write/send actions.

## Examples
### Good example patterns
1. State whether the change concerns ServicePlanner Service Assistant or an EmployeeAgent flow before editing.
2. Verify ServicePlanner user permissions, bot permissions, and Case access before changing metadata.

### Bad examples / avoid
1. Copying EmployeeAgent `.agent` script patterns into a ServicePlanner bundle.
2. Treating an activation/publish step as normal validation without explicit approval.

## Review checklist
- [ ] Change scope is minimal and component-appropriate.
- [ ] ServicePlanner vs EmployeeAgent ownership is explicit.
- [ ] Rep and bot user permission requirements are reviewed.
...
|---|---|---|
| Conflating ServicePlanner with `.agent` DSL agents | Produces invalid metadata and wrong troubleshooting paths | Identify ServicePlanner vs EmployeeAgent before editing |
| Checking only the visible rep permissions | ServicePlanner execution can also fail under the bot user | Verify both rep and bot user permission sets |
| Assuming activation is allowed | Activation can change live assistant behavior | Require explicit approval for deploy/publish/activate |
...
|---|---|
| Agent Script DSL grammar, action types, transitions, anti-patterns | [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) |
| Topic instruction authoring, sectioning, guardrails phrasing | [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) |
| Generate / Validate / Deploy / Publish / Activate lifecycle, publish-then-override | [../salesforce-agentforce-authoring-bundle/SKILL.md](../salesforce-agentforce-authoring-bundle/SKILL.md) |
| Builder UI " DSL parity, ServicePlanner bot metadata, permission personas | [../salesforce-agentforce-builder/SKILL.md](../salesforce-agentforce-builder/SKILL.md) |
...
""" localTopicLinks Ã-- N (one per subagent)
""" localTopics Ã-- N (INLINE subagent definitions)
"‚     """" localActions Ã-- M (inline GenAiFunction wrappers)
...
6. Build + activate the eligibility flow (see Section 2 6)
...
13. Assign permission sets to rep, bot user, admin (Section 2 4)
...
For instruction-authoring rules (one task per instruction, guardrails at `sortOrder=1`, utterance counts, etc.) see [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md).
...
For full lifecycle commands (generate, validate, deploy, publish, activate, retrieve) see [../salesforce-agentforce-authoring-bundle/SKILL.md](../salesforce-agentforce-authoring-bundle/SKILL.md). ServicePlanner-specific deltas:
...
| API version | 66.0 |
| Apex controller | `CaseEmailWorkspaceController` (see Section 3 4) |
...
For full mechanics see [../salesforce-agentforce-authoring-bundle/SKILL.md Section 8 Empirical Finding #7](../salesforce-agentforce-authoring-bundle/SKILL.md). Plusgrade-specific manifest:
...
For any EmployeeAgent processing PII, all three must align (anti-pattern #9 in `../salesforce-agentforce-authoring-bundle/SKILL.md`):
...
3. `system.instructions` includes an explicit PII redaction policy (yes present in `Email_Analysis_Agent.agent` -- DATA PRIVACY section)
...
| Confirmation candidate present but malformed | `INVALID_CONFIRMATION_FORMAT` | none -- surfaces in `Agent_Activity_Log__c.Decision_Code__c`; LWC tooltip pending (Section 3 7) |
...
- [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) -- DSL grammar (the only place to look up syntax)
- [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) -- topic-instruction authoring rules
- [../salesforce-agentforce-authoring-bundle/SKILL.md](../salesforce-agentforce-authoring-bundle/SKILL.md) -- lifecycle, publish-then-override, bot-meta gotchas
- [../salesforce-agentforce-builder/SKILL.md](../salesforce-agentforce-builder/SKILL.md) -- Builder UI " source parity, ServicePlanner bot metadata
