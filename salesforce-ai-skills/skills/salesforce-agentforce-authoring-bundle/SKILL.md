...
- Project default target environment set: `sf config set target-org <target-env-alias>`
- Agentforce license enabled in the target environment
...
Then:
```bash
sf project deploy start --json --manifest manifest/package-email-analysis-bot-override.xml --target-org <target-env-alias>
```
