...
description: Production Salesforce AI skill for Access model, permission sets/groups, muting, external auth access.
license: Apache-2.0
compatibility:
  - Claude Code
  - Claude Agents
...
## TRIGGER when
- The task involves Access model, permission sets/groups, muting, external auth access.
...
- Also read: Global Development + Metadata + Deployment.
...
Prefer Permission Sets over Profile edits; aggregate in Permission Set Groups. Use Muting Permission Sets to reduce elevated permissions. Validate object/field/class/flow/tab/app/external credential access.

## Upstream Salesforce Skill Patterns
- Permission sets need a clear API name, label, description, intended persona, and least-privilege justification.
- Do not add required fields to `<fieldPermissions>`; required fields cannot receive FLS entries and can fail deployment.
- Formula fields are never editable. Master-detail fields are required on the detail side.
- Use exact field references as `ObjectApiName.FieldApiName`; custom object tabs use the custom object API name with `__c`, while standard object tabs use the platform tab naming convention.
- Security-review broad system permissions such as `ViewAllData`, `ModifyAllData`, and `ManageUsers`.
- For Agentforce Employee Agent access, use `agentAccesses` with the agent developer name and verify the agent exists before granting access.
- Validate for duplicate permissions, wrong API names, missing object/field dependencies, and persona overreach before deployment.

## Examples
### Good example patterns
1. Feature access delivered via dedicated perm set and PSG assignment.
2. External Credential principal access granted only to required integration users.

### Bad examples / avoid
1. Granting broad Modify All Data for convenience.
2. Embedding profile-name logic in runtime bypass decisions.
...
Permission Sets (capability layers -- Object/Field/Apex/Flow/Tab/App/CustomPerm)
   +
Permission Set Group (role bundle = N component PS ˆ' Muting PS)
```
...
- `agentName` must exactly match the agent's `developer_name` (case-sensitive, see [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) Section 5 Service Agents (`AgentforceServiceAgent`) use a different access path -- the system PS `AgentforceServiceAgentUser` plus channel routing. See Section 6 below.
...
A PSG bundles N component PS into one assignable unit. Effective permissions = union(component PS) ˆ' muting PS. Salesforce caches the result and recalculates on status change. Assign the PSG to users; never assign components directly when a PSG exists for that role.
...
The system PS `AgentforceServiceAgentUser` ships with Salesforce and provides Messaging/channel infrastructure. Your custom PS supplies the agent-specific class/flow/object grants. The bot user (`config.default_agent_user` per [../salesforce-agentforce-script/SKILL.md](../salesforce-agentforce-script/SKILL.md) Section 5 MUST have both PS assigned.
