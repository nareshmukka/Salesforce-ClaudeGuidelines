...
description: Production Salesforce AI skill for HTTP callouts, Platform Events, CDC, external auth/data sync.
license: Apache-2.0
compatibility:
  - Claude Code
  - Claude Agents
...
## TRIGGER when
- The task involves HTTP callouts, Platform Events, CDC, external auth/data sync.
...
- Also read: Global Development + Apex + Observability + Testing.
...
Use Named Credentials/External Credentials; never embed secrets. Define request/response DTOs. Set explicit timeouts, retries with backoff, and idempotency keys. Log with correlation IDs.

## Upstream Salesforce Skill Patterns
- For Agentforce-backed actions, expose integrations through invocable Apex, autolaunched Flow, or prompt/action metadata with explicit request and response contracts.
- Return structured error fields instead of opaque strings when integrations feed Flow, Agentforce, or LWC automation.
- Make external writes idempotent with a stable key, bounded retry policy, and duplicate-safe handling.
- Keep human-facing summaries separate from machine-readable outputs when an agent or LWC parses the response.
- Test success, handled error, timeout, malformed response, and permission-denied paths with mocks before deployment.

## Examples
### Good example patterns
1. Callout client maps JSON into typed DTO and captures status/body safely.
2. Platform Event payload includes correlationId + sourceSystem for traceability.

### Bad examples / avoid
1. Parsing response with fragile string operations.
2. Retry loop without cap/backoff causing duplicate writes.
...
| Condition | Action |
|---|---|
| 1 Ã-- HTTP 401/403 in last 5 min | Page ops immediately -- credential expired or revoked |
| >=3 Ã-- Failure on same integration in 5 min | Page ops -- external system likely down |
