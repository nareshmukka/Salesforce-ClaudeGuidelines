...
description: Production Salesforce AI skill for Apex classes, invocables, async jobs, selectors/services.
license: Apache-2.0
compatibility:
  - Claude Code
  - Claude Agents
...
## TRIGGER when
- The task involves Apex classes, invocables, async jobs, selectors/services.
...
- Also read: Global Development + Trigger + Testing + Integration.
...
Class header includes purpose, owner, and dependencies. Prefer inherited sharing for service classes, with sharing for controllers. Use service + selector + DTO patterns. Bulkify all DML/queries. Use Database.SaveResult for partial success when required.

## Upstream Salesforce Skill Patterns
- Classify Apex before editing: service, selector, domain, batch, queueable, schedulable, invocable, DTO, utility, interface, exception, REST resource, trigger, or trigger action.
- Hard stops: no SOQL/DML in loops, explicit sharing on executable classes, no hardcoded IDs/secrets, bind user-controlled dynamic SOQL, no Java collection types, and no new `@future` methods.
- Default architecture: trigger routes only; service coordinates; selector owns SOQL; domain owns in-memory business rules; integration layer owns HTTP and external DTOs.
- Security defaults: `with sharing` for entry points, `inherited sharing` for selectors where appropriate, `WITH USER_MODE` for user-context SOQL, and `AccessLevel.USER_MODE` for DML when CRUD/FLS must be honored.
- Invocable methods: accept `List<Request>`, return `List<Response>`, include `isSuccess`, `errorMessage`, and `errorType`, and reserve thrown exceptions for unrecoverable Flow fault paths.
- Async selection: prefer Queueable with Finalizer for standard async work, Batch for very large datasets, Scheduled Flow for simple recurring work, and Continuation for long-running callouts.

## Examples
### Good example patterns
1. Selector class returns typed records with field set constants.
2. Queueable for chained non-batch async callout workloads with limits awareness.

### Bad examples / avoid
1. SOQL/DML in loops or static Boolean recursion toggles across transactions.
2. Hardcoded endpoint/token or allOrNone=true when partial success is required.
...
## Apex-specific implementation rules
- Class header standard: Purpose, Owner, Last Updated, Dependencies.
- Use `inherited sharing` for services unless stricter `with sharing` is required.
- Prefer selector classes for query reuse; avoid inline SOQL sprawl.
- Invocable methods: stable request/response DTOs, bulk-safe input list handling.
- Async selection: Future (legacy/simple), Queueable (preferred async orchestration), Batch (large datasets).
- Use `Database.insert(records, false)` / `Database.update(records, false)` when partial success is desired and handle `SaveResult`.
- Tests: positive, negative, bulk, and security context assertions.
...
Sharing keywords control **record-level visibility**. They do NOT enforce FLS or CRUD -- those are separate (Section 6 Keyword | Behaviour | Use when |
|---|---|---|
